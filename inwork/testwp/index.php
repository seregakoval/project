<!doctype html>
<html lang="ru">
<head>
    <meta charset="UTF-8">
    <title>testwp</title>
    <!--<link rel="stylesheet" href="style.css">-->
    <link href="http://allfont.ru/allfont.css?fonts=europe-bold" rel="stylesheet" type="text/css" />

</head>
<body>
<div class="header">
    <div class="wrapper">
        <div class="top clearfix">
            <div class="nav">
                <ul>
                    <li><a href="#" class="active">Главная</a></li>
                    <li><a href="#">Галерея</a></li>
                    <li><a href="#">Контакты</a></li>
                </ul>
            </div>
            <div class="tel">
                <p><span><img src="img/whatsapp.png" alt=""></span><span><img src="img/tel.png" alt=""></span>8 923 460 8362</p>
                <div style="clear: both;"></div>
                <p><span><img src="img/whatsapp.png" alt=""></span><span><img src="img/tel.png" alt=""></span>8 950 574 2861</p>
            </div>
        </div>
        <h1>Покупаем бивни мамонта, <br>
            рога и клыки древних животных</h1>
        <p class="sub-text">Скупка бивней мамонта, рогов шерстистого носорога, клыков моржа, рогов лося и оленя <br>
            и их фрагментов с самовывозом и предварительной оценкой <br>
            по фото в течение 15 минут </p>
        <div class="buttons clearfix">
            <a href="#" class="but-left">Оценить по фото</a>
            <a href="#" class="but-right">Получить консультацию</a>
        </div>
    </div>
</div>
<div class="sell clearfix">
    <h1>Мы продаем</h1>
    <p class="sub-text">Коллекционные бивни, клыки, рога, их фрагменты и бой</p>
    <div class="boxes clearfix">
        <div class="box">
            <img src="img/img1.jpg" alt="">
            <p>Бивни мамонта</p>
        </div>
        <div class="box">
            <img src="img/img2.jpg" alt="">
            <p>Клыки моржа</p>
        </div>
        <div class="box">
            <img src="img/img3.jpg" alt="">
            <p>Рога шерстистого носорога</p>
        </div>
        <div class="box">
            <img src="img/img4.jpg" alt="">
            <p>Рога лосей и Оленей</p>
        </div>
    </div>
</div>
<div class="block-work">
    <div class="work clearfix">
        <h1>Как мы работаем?</h1>
        <p class="sub-text">Мы занимаемся скупкой не для последующей перепродажи, а для изготовления красивых <br>
            и качественных изделий</p>
        <div class="content clearfix">
            <div class="left-content">
                <h2>01. Свяжитесь с нами</h2>
                <p class="text">Вам необходимо оставить заявку по телефону 8 923 460 8362 <br> или на нашем сайте, специалист свяжется с Вами в ближайшее время.</p>
                <a href="#" class="btn">оставить заявку</a>
                <h2>02. Отправьте фото или договоритесь о встрече</h2>
                <p class="text">Отправьте фотографии для расчета любым удобным способом, либо договоритесь о встрече с нашим специалистом и договоритесь о цене</p>
                <h2>03. Получите оплату</h2>
                <p class="text">Мы производим оплату наличными при встрече или перечисляем на банковскую карту или указанный Вами счет</p>
                <h2>04. Отправьте или передайте материал</h2>
                <p class="text">Передайте материал нашим специалистам (при самовывозе) или отправьте любой удобной транспортной компанией</p>
            </div>
            <img src="img/img5.png" class="img" alt="">
        </div>
    </div>
</div>
<div class="form-block clearfix">
    <div class="wrapper">
        <div class="left-content">
            <h1>Отправьте Фотографии</h1>
            <p class="sub-text">Наш специалист свяжется с Вами и оценит дистанционно
                стоимость и качество материала в течение 15 минут</p>
            <div class="button clearfix">
                <a href="#" class="button1">Заявка на расчет</a>
                <a href="#" class="button1">Заявка на расчет</a>
            </div>
        </div>
        <div class="form">
            <h1>Заявка на расчет</h1>
            <form action="#">
                <input type="text" placeholder="Ваше Имя">
                <input type="text" placeholder="Ваше Имя">
                <p class="text-f">Прикрепить фотографии:</p>
                <div class="photo clearfix">
                    <ul>
                        <li><a href="#"><img src="img/photo.png" alt=""></a></li>
                        <li><a href="#"><img src="img/photo.png" alt=""></a></li>
                        <li><a href="#"><img src="img/photo.png" alt=""></a></li>
                        <li><a href="#"><img src="img/photo.png" alt=""></a></li>
                    </ul>
                </div>
                <div class="input">
                    <input type="text" placeholder="Размер">
                    <input type="text" placeholder="Вес">
                </div>
                <input type="submit" value="Рассчитать" class="submit">
            </form>
        </div>
    </div>
</div>
<div class="info clearfix">
    <h1>Итак, у вас есть материал</h1>
    <p class="sub-text">Почему его нужно продать именно нам?</p>
    <div class="boxes clearfix">
        <div class="box">
            <div class="img">
                <img src="img/img6.png" alt="" class="top-img">
            </div>
            <p>Мы гарантируем реальную цену <br> за материал, порядочность и <br> оплату в срок</p>
        </div>
        <div class="box">
            <div class="img">
                <img src="img/img7.png" alt="" class="top-img">
            </div>
            <p>Мы называем точную цену и не <br> пытаемся ее сбить в 2-3 раза, <br> ссылаясь на недостатки материала</p>
        </div>
        <div class="box">
            <div class="img" >
                <img src="img/img8.png" alt="">
            </div>
            <p>Мы ориентированы на <br> постоянное сотрудничество и <br> предложим спецусловия на <br> повторные продажи</p>
        </div>
    </div>
    <a href="#" class="btn">Начать сотрудничество</a>
</div>
<div class="top-f">
    <h1>Остались вопросы?</h1>
    <p class="sub-text">Задайте их вашему специалисту</p>
</div>
<div class="footer clearfix">
    <div class="bottom-block clearfix">
        <div class="nav">
            <ul>
                <li><a href="#">Главная</a></li>
                <li><a href="#">Галерея</a></li>
                <li><a href="#">Контакты</a></li>
            </ul>
        </div>
        <div class="tel">
            <p><span><img src="img/whatsapp-black.png" alt=""></span><span><img src="img/tel-black.png" alt=""></span>8 923 460 8362</p>
            <div style="clear: both;"></div>
            <p><span><img src="img/whatsapp-black.png" alt=""></span><span><img src="img/tel-black.png" alt=""></span>8 950 574 2861</p>
        </div>
        <a href="#" class="cell-btn">Заказать звонок</a>
    </div>
</div>
</body>
</html>