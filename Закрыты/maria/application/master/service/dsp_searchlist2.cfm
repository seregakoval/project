...
<head>
	...
	<script>
		...
		function deleterecode(formhdrid,thispagenumber,parentid){
			var url="databind/act_checkdelete2.cfm?formhdrid=" + formhdrid ;
			//prompt('',url);
			...
			$(xml).find('returnrow').each(function(index){
				...
				if(msgerror == ""){ 	
					if(confirm("Are you sure you want to delete this recode?")){
						var unk = new Date();
						document.location="index.cfm?fuseaction=deletemaingroup&formhdrid=" + formhdrid + "&thispagenumber=" + thispagenumber + "&parentid=" + parentid + "&unk=" + unk;
					}				
				}
				else{
...
<cfquery name="getrecodecount" datasource="#dbName#"><!--- CHANGE ACCORDING TO YOUR MASTER --->
	...
	from maingroup_pos 					 <!--- ***CHANGEHERE : rename table --->
	where itemtype='2'
			...
			<tr>
				...
				<cfif searchtype neq "3"><!--- THIS TYPE MEAN PRINT LIST --->
					<td class="datatabledatacell_#(getData.currentRow%2)#" style="text-align:center;">
						<img src="/symphony/v33/images/newimg/edit8.png" class="imgtoplink" title="Edit this recorde" onclick="parent.setedit('#getData.groupid#','#getData.code#','#encodeForJavaScript(getData.description)#','#getData.scid#');">		<!--- ***CHANGEHERE : rename column & remove or add variable name --->				
						<img src="/symphony/v33/images/newimg/dele.png" class="imgtoplink" title="Delete this recorde" onclick="deleterecode('#getData.groupid#','#thispagenumber#','#parentid#');">					<!--- ***CHANGEHERE : rename column --->
					</td>
				</cfif>
			</tr>
		</cfloop>
