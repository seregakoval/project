...
<head>
	...
	<script>
		...
		function deleterecode(formhdrid,thispagenumber,parentid){
			var url="databind/act_checkdelete4.cfm?formhdrid=" + formhdrid ;
			//prompt('',url);
			...
			$(xml).find('returnrow').each(function(index){
				...
				if(msgerror == ""){
					if(confirm("Are you sure you want to delete this recode?")){
						var unk = new Date();
						document.location="index.cfm?fuseaction=deleteitemcate&formhdrid=" + formhdrid + "&thispagenumber=" + thispagenumber + "&parentid=" + parentid + "&unk=" + unk;
					}
				}
				else{
		...
		function topWindow(url){
			powindow = window.open(url,'powindow','width=420,height=300,top=20,left=50,scrollbars=yes,resizable=yes,status=no');
			powindow.window.focus();
		}	

		function edititem(formhdrid){
			// ***CHANGEHERE : remove or add variable as required &variablename=variable
			<cfoutput>
			var url ="index.cfm?fuseaction=edititem&parentid=#parentid#&formhdrid=" + formhdrid;
			</cfoutput>
...
<cfquery name="getrecodecount" datasource="#dbName#"><!--- CHANGE ACCORDING TO YOUR MASTER --->
	...
	from itemcate_pos 					 <!--- ***CHANGEHERE : rename table --->
	where itemtype='1'
	<!--- ***CHANGEHERE : add or remove <cfif>....</cfif> as required --->
	<cfif searchcode NEQ "" and trim(searchcode) NEQ "Code">
		and upper(codeno) like '%#ucase(searchcode)#%'			<!--- ***CHANGEHERE : rename column --->
	</cfif>
...
<cfquery name="getData" datasource="#dbName#"><!--- CHANGE ACCORDING TO YOUR MASTER --->
	select itemid,description,codeno as code
	from itemcate_pos 		<!--- ***CHANGEHERE : rename table --->
	where itemtype='1'
	<!--- ***CHANGEHERE : add or remove <cfif>...</cfif> as required --->
	<cfif searchcode NEQ "" and trim(searchcode) NEQ "Code">
			...
			<tr>
				...
				<cfif searchtype neq "3"><!--- THIS TYPE MEAN PRINT LIST --->
					<td class="datatabledatacell_#(getData.currentRow%2)#" style="text-align:center;">
						<img src="/symphony/v33/images/newimg/edit8.png" class="imgtoplink" title="Edit this recorde" onclick="edititem('#getData.itemid#');">		<!--- ***CHANGEHERE : rename column & remove or add variable name --->
						<img src="/symphony/v33/images/newimg/dele.png" class="imgtoplink" title="Delete this recorde" onclick="deleterecode('#getData.itemid#','#thispagenumber#','#parentid#');">					<!--- ***CHANGEHERE : rename column --->
						<cfif session.v33.master_changestockunit eq "1">
							<img src="/symphony/v33/images/newimg/infogreen.png" class="imgtoplink" title="Edit Stock unit" onclick="topWindow('index.cfm?fuseaction=changeunit&formhdrid=#getData.itemid#');">
						</cfif>
					</td>
				</cfif>
			</tr>
