...
<head>
	...
	<script>
		function searchrec(searchtype,thispagenumber){
			...
			searchframe.location = "dsp_searchlist2.cfm?searchcode=" + searchcode + "&searchdesc=" + searchdesc + "&searchtype=" + searchtype + "&thispagenumber=" + thispagenumber + "&sortby=" + sortby + "&parentid=" + parentid + "&unk=" + unk;
		
		}
		
		// ***CHANGEHERE : remove or add variable as required
		function setedit(docid,code,descp,scid){
			// ***CHANGEHERE : remove or add variable as required
			document.itemform.fuseaction.value="updatemaingroup";
			document.itemform.docid.value=docid;
			document.itemform.code.value=code;
			document.itemform.descp.value=descp;
			document.itemform.supcategory.value=scid;
		...
		function validateform(){
			...
			var supcategory = document.itemform.supcategory.value;
			var rtn = true;
			// ***CHANGEHERE : remove or add variable as required
			<cfoutput>
			if(((code == "" || code == "Code") && "#global_mstmaingroupautonumber#" == "manual") || (descp == "" || descp == "Description")){
				alert("Code or Description Can not be empty!!!");
				rtn = false;			
			}
			else if(supcategory == ""){
				alert("Supplier Category Can not be empty!!!");
				rtn = false;
			}
			else{
				...
				// ***CHANGEHERE : remove or add variable as required &variablename=variable
				var url="databind/act_formcheck2.cfm?code=" + code + "&descp=" + descp + "&docid=" + docid + "&fuseaction=" + fuseaction;
				//prompt('',url);
				...
				$(xml).find('returnrow').each(function(index){
					...
					if(msgerror != ""){ 	
						...
						var parentid = document.itemform.parentid.value;
						document.location = "index.cfm?fuseaction=" + fuseaction + "&code=" + code + "&descp=" + descp + "&supcategory=" + supcategory + "&docid=" + docid + "&level=" + level + "&parentid=" + parentid;
					}
		...
		function setdefault(){
			// ***CHANGEHERE : remove or add variable as required
			document.itemform.fuseaction.value="submitmaingroup";
			document.itemform.docid.value="";
			document.itemform.code.value="Code";
			document.itemform.descp.value="Description";
			document.itemform.supcategory.value="";
...
<cfoutput>
<cfquery name="getsupp" datasource="#dbname#">
	select * from suppcate
	order by description
</cfquery>
<body class="mainpage">
<form name="itemform" method="post" action="index.cfm" style="height:100%;">
	<input type="hidden" name="fuseaction" value="submitmaingroup">	<!--- ***CHANGEHERE : change action  as required --->
	...
	<table class="basetable">
			...
			<td colspan="100%" style="border-bottom:0px solid ##cccccc;vertical-align:top;text-align:left;height:20px;" id="addnewcell">
				<div  class="addnewtable">
					<table style="margin-top:28px;margin-left:25px;position:absolute;">
						...
						<tr>
							<td style="width:60px;">
								<input maxlength="50" type="text" class="textboxnormal" style="width:70px;" name="code" value="Code" onclick="if(this.value=='Code'){this.value = ''};" onblur="if(this.value==''){this.value = 'Code'};" <cfif global_mstmaingroupautonumber neq "manual">readonly </cfif>>												
							</td>
							<td style="width:150px;">
								<input maxlength="150" type="text" class="textboxnormal" style="width:150px;" name="descp" value="Description" onclick="if(this.value=='Description'){this.value = ''};" onblur="if(this.value==''){this.value = 'Description'};" >
							</td>
							<td style="width:130px;">
								<select name="supcategory" class="textboxnormal" style="width:130px;">
									<option value="">Supplier Category</option>
									<cfloop query="getsupp">
										<option value="#getsupp.scid#">#getsupp.description#</option>
									</cfloop>
								</select>
							</td>
							<td>
		...
		<tr><!--- START BOTTOM IFRAME ROW --->
			<td colspan="100%" style="height:100%;margin:0;text-align:left;">
				<iframe id="searchframe" name="searchframe" seamless src="dsp_searchlist2.cfm?thispagenumber=1&searchtype=1&parentid=#parentid#" style="height:100%;width:100%;border:0px solid ##647796;overflow:auto;vertical-align:top;"></iframe>
			</td>
