...
<head>
	...
	<script>
		function searchrec(searchtype,thispagenumber){
			...
			searchframe.location = "dsp_searchlist.cfm?searchcode=" + searchcode + "&searchdesc=" + searchdesc + "&searchtype=" + searchtype + "&thispagenumber=" + thispagenumber + "&sortby=" + sortby + "&parentid=" + parentid + "&unk=" + unk;
		...
		function setedit(docid,code,descp){
			// ***CHANGEHERE : remove or add variable as required
			document.itemform.fuseaction.value="updatemainlevel";
		...
		function validateform(){
			...
			// ***CHANGEHERE : remove or add variable as required
			 <cfoutput>
			if(((code == "" || code == "Code") && "#global_mstmainlevelautonumber#" == "manual") || (descp == "" || descp == "Description")){
				...
				// ***CHANGEHERE : remove or add variable as required &variablename=variable
				var url="databind/act_formcheck.cfm?code=" + code + "&descp=" + descp + "&docid=" + docid + "&fuseaction=" + fuseaction;
				//prompt('',url);
		...
		function setdefault(){
			// ***CHANGEHERE : remove or add variable as required
			document.itemform.fuseaction.value="submitmainlevel";
...
<form name="itemform" method="post" action="index.cfm" style="height:100%;" onsubmit="return validateform();">
	<input type="hidden" name="fuseaction" value="submitmainlevel">	
	...
	<table class="basetable">
			...
			<td colspan="100%" style="border-bottom:0px solid ##cccccc;vertical-align:top;text-align:left;height:20px;" id="addnewcell">
				<div  class="addnewtable">
					<table style="margin-top:28px;margin-left:25px;position:absolute;">
						...
						<tr>
							<td style="width:60px;">
								<input type="text" class="textboxnormal" style="width:70px;" name="code" value="Code" onclick="if(this.value=='Code'){this.value = ''};" onblur="if(this.value==''){this.value = 'Code'};" <cfif global_mstmainlevelautonumber neq "manual">readonly </cfif>>												
							</td>
							<td style="width:200px;">
								<input type="text" class="textboxnormal" style="width:200px;" name="descp" value="Description" onclick="if(this.value=='Description'){this.value = ''};" onblur="if(this.value==''){this.value = 'Description'};" >
							</td>
							<td>
		...
		<tr><!--- START BOTTOM IFRAME ROW --->
			<td colspan="100%" style="height:100%;margin:0;text-align:left;">
				<iframe id="searchframe" name="searchframe" seamless src="dsp_searchlist.cfm?thispagenumber=1&searchtype=1&parentid=#parentid#" style="height:100%;width:100%;border:0px solid ##647796;overflow:auto;vertical-align:top;"></iframe>
			</td>
