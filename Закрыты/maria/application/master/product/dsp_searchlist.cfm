...
<head>
	...
	<script>
		...
		function deleterecode(formhdrid,thispagenumber,parentid){
			var url="databind/act_checkdelete.cfm?formhdrid=" + formhdrid ;
			//prompt('',url);
			...
			$(xml).find('returnrow').each(function(index){
				...
				if(msgerror == ""){ 	
					if(confirm("Are you sure you want to delete this record?")){
						var unk = new Date();
						document.location="index.cfm?fuseaction=deletemainlevel&formhdrid=" + formhdrid + "&thispagenumber=" + thispagenumber + "&parentid=" + parentid + "&unk=" + unk;
					}				
				}
				else{
...
<cfquery name="getrecodecount" datasource="#dbName#"><!--- CHANGE ACCORDING TO YOUR MASTER --->
	...
	from mainlevel_pos					 <!--- ***CHANGEHERE : rename table --->
	where itemtype='1'
