<cfsilent>
	...
	<cfif isdefined("fuseaction") and trim(fuseaction) neq "">	
		<cfif trim(code) eq "" and global_mstmainlevelautonumber eq "manual">
			<cfset val = "Please Enter Code #tabchr#">
		</cfif>
		<cfif fuseaction eq "submitmainlevel"><!--- WHEN ADD NEW DATA --->
			<cfquery name="chkdublicate" datasource="#dbName#"><!--- check dublicate data CHANGE ACCORDING TO YOUR MASTER--->
				select * 
				from mainlevel								<!--- ***CHANGEHERE : rename table --->
				where 
				<cfif global_mstmainlevelautonumber eq "manual">
		...
		<cfelseif fuseaction eq "updatemainlevel"><!--- WHEN EDIT DATA --->
			<cfquery name="chkdublicate" datasource="#dbName#"><!--- check dublicate data CHANGE ACCORDING TO YOUR MASTER --->
				select * 
				from mainlevel									<!--- ***CHANGEHERE : rename table --->
				where  id != '#docid#'					<!--- ***CHANGEHERE : add or remove column & rename column--->
				and upper(code) = '#ucase(code)#'				<!--- ***CHANGEHERE : add or remove column & rename column--->
				or (upper(description) = '#ucase(descp)#' and id != '#docid#')		<!--- ***CHANGEHERE : rename column --->		
			</cfquery>
			<cfif chkdublicate.RecordCount gt 0>
				<cfset val = "This Data already got!!! #tabchr#">
			</cfif>
