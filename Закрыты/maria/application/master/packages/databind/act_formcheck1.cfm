<cfsilent>
	...
	<cfif isdefined("fuseaction") and trim(fuseaction) neq "">	
		<cfif trim(code) eq "" and global_mstmainitemautonumber eq "manual">
			<cfset val = "Please Enter Code. #tabchr#">
		</cfif>
		<cfif fuseaction eq "submitmainitem"><!--- WHEN ADD NEW DATA --->
			<cfquery name="chkdublicate" datasource="#dbName#"><!--- check dublicate data CHANGE ACCORDING TO YOUR MASTER--->
				...
				where  
				<cfif global_mstmainitemautonumber eq "manual">
		...
		<cfelseif fuseaction eq "updatemainitem"><!--- WHEN EDIT DATA --->
			<cfquery name="chkdublicate" datasource="#dbName#"><!--- check dublicate data CHANGE ACCORDING TO YOUR MASTER --->
				select * 
				from mainitem									<!--- ***CHANGEHERE : rename table --->
