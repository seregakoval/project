...
function my_plugin_menu() {
	//add_options_page( 'XML to DataBase','XML to DataBase','XML_to_DataBase','options_page_slug', 'xml_settings_page');
	add_submenu_page('tools.php',  'XML to DataBase','XML to DataBase','edit_plugins', 'XML_to_DataBase', 'xml_settings_page');
}
function ajax_save_xml(){
	if(isset($_POST['xml_file'])){
		update_option('xml_file', $_POST['xml_file']);
		$curl_handle = curl_init();
		curl_setopt($curl_handle, CURLOPT_URL, $_POST['xml_file']);
		//curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 2);
		curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
		$str = curl_exec($curl_handle);
		curl_close($curl_handle);
		if($str)
			file_put_contents(__DIR__ . '/data.xml', $str);
	}
...
add_action('wp_ajax_save_xml', 'ajax_save_xml');
//add_action('wp_ajax_nopriv_xml_save', 'ajax_save_xml');
add_action('wp_ajax_save_sync', 'ajax_save_sync');
//add_action('wp_ajax_nopriv_xml_sync', 'ajax_save_sync');

function  xml_settings_page (){
	$res = array();
	?>
	<?php if($res){?>
	<div>
		<div>
			Update: <?php echo $res[1]?>
		</div>
		<div>
			Insert: <?php echo $res[0]?>
		</div>
	</div>
		<?php }?>
	<form method="post">
		<div class="row">
			<div>
				XML file <input style="width: 50%" type="text" name="xml_file" value="<?php echo get_option('xml_file') ?>">
			</div>
			<div>
				<div>LOG:</div> <textarea readonly rows="5" id="log" cols="50"></textarea>
			</div>
			<div>
				<input type="button" name="sync" value="Sync" id="xml_sunc">
			</div>
		</div>
	</form>
	<script type="text/javascript">
	jQuery('#xml_sunc').click(function(){
		var log = jQuery('#log');
		log.val(log.val()+'Start load xm file...\n');
		jQuery.ajax({
			type:'post',
			url:'<?php echo admin_url('admin-ajax.php') ?>',
			data:{action: 'save_xml', xml_file: jQuery('input[name="xml_file"]').val()},
			success: function(d){
				log.val(log.val()+'XML file loaded...\n');
				log.val(log.val()+'Start sync XML file...\n');
				jQuery.ajax({
					type:'post',
					url: '<?php echo admin_url('admin-ajax.php') ?>',
					data: {action: 'save_sync'},
					dataType:'json',
					success: function(){
						log.val(log.val()+'End sync XML file...\n');
					},
					error: function(){
						log.val(log.val()+'Error sync XML file...\n');
					}

				});
			},
			error: function(){
				log.val(log.val()+'XML file not loaded...\n');
			},
			dataType:'json'
		});
	});
...
function ajax_save_sync($file){
	try {
		...
		$str = file_get_contents(__DIR__ .'/data.xml');
		$countInsert = 0;
		$countUpdate = 0;
		if ($str) {
			//return;
			...
			$xml->read();
			var_dump($xml->name);
			//var_dump($xml->moveToElement());
			var_dump($xml->next('property'));
			var_dump($xml->name);
			var_dump($xml->getAttribute('id'));*/
			//var_dump($xml);
			/*var_dump($xml->property[0]->id);
			exit;*/
			$wpdb->query('DELETE from `features`');
			$wpdb->query('DELETE from `img`');
			$wpdb->query('DELETE from `feed`');
			foreach ($xml->property as $el) {
				//if ($el->getName() != 'property') continue;
				//$el = $el->children();
				$id = preg_replace('#\D#u', '', $el->id);
//			var_dump(intval($id));
				$date = $el->date;
//			var_dump($date);
				$ref = $el->ref;
//			var_dump($ref);
				...
				$price_freq = $el->price_freq;
				$part_ownership = $el->part_ownership;
				$leasehold = $el->leasehold;
				$new_build = $el->new_build;
				$type = $el->type;
				$town = $el->town;
				$beds = intval($el->beds);
				$baths = intval($el->baths);
				$pool = intval($el->pool);
				$feature = $el->features;
				$text = $el->desc->en;
				$textfr = $el->desc->fr;
				$textes = $el->desc->es;
				//print_r($text);
				$surb = $el->surface_area->built;
				$surp = $el->surface_area->plot;
				$imgsrc = $el->images;
				//	$feature = (array)$features;
				$res = $wpdb->query($wpdb->prepare('select id from feed where id = %d', array($id)));

				//if($wpdb->last_error) continue;
				if ($res) {
					$sql = $wpdb->prepare('UPDATE feed SET `date`=%s,`ref`=%s,`price`=%s,`currency`=%s,`price_freq`=%s,`part_ownership`=%s,`leasehold`=%s,`new_build`=%s,`type`=%s,`town`=%s,`beds`=%d,`baths`=%d,`pool`=%d,built=%s,plot=%s,textfr=%s,textes=%s
					WHERE id=%d', array($date, $ref, $price, $currency, $price_freq, $part_ownership, $leasehold, $new_build, $type, $town, $beds, $baths, $pool, $surb, $surp, $textfr, $textes, $id));
					$res = $wpdb->query($sql);
//			$wpdb->query('DELETE from `features` where id = '.$id);
//			$wpdb->query('DELETE from `img` where id = '.$id);
//			$wpdb->query('DELETE from `feed` where id = '.$id);
					if ($res)
						$countUpdate++;
					...
					if (is_array($feature->feature)) {
						foreach ($feature->feature as $two) {

							$name = "feature" . $i;
							//$$name = $two;
							$i++;
							$sql = $wpdb->prepare("INSERT INTO `features` (`id`,`feature`) VALUES (%d,%s)", array($id, $two));
							$wpdb->query($sql);
						}
					}

					$idimg = 1;
					if (is_array($imgsrc->image)) {
						foreach ($imgsrc->image as $imgn) {
							if (!empty($imgn->url)) {
								foreach ($imgn->url as $tree) {

									$sql = $wpdb->prepare("INSERT INTO `img` (`id`,`idimg`,`src`) VALUES (%d,%d,%s)", array($id, $idimg, $tree));
									$wpdb->query($sql);
									$idimg++;
					...
					$sql = $wpdb->prepare("INSERT INTO `feed` (`id`, `date`,`ref`,`price`,`currency`,`price_freq`,`part_ownership`,`leasehold`,`new_build`,`type`,`town`,`beds`,`baths`,`pool`,built,plot,`text`,`textfr`,`textes`) VALUES (%d,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%d,%d,%d,%s,%s,%s,%s,%s)", array($id, $date, $ref, $price, $currency, $price_freq, $part_ownership, $leasehold, $new_build, $type, $town, $beds, $baths, $pool, $surb, $surp,$text,$textfr,$textes));
					$res = $wpdb->query($sql);
					if ($res !== FALSE) {
						$countInsert++;
						$i = 0;
				...
				//echo $res.' asdasd '. $id . '<br>';
				//$textt = $text->en;
				//print_r($text);
				/*$query5 = "UPDATE feed SET built='".$surb."', plot='".$surp."'
						WHERE id=".$id."";*/
				//$res5 = mysql_query($query5);


				//		$query = "INSERT INTO `feed` (`id`, `date`,`ref`,`price`,`currency`,`price_freq`,`part_ownership`,`leasehold`,`new_build`,`type`,`town`,`beds`,`baths`,`pool`)
				//	VALUES ('$id', '$date','$ref','$price','$currency','$price_freq','$part_ownership','$leasehold','$new_build','$type','$town','$beds','$baths','$pool')";
	...
	echo json_encode(array($countInsert,$countUpdate));
	wp_die();
}
